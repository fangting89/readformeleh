...
- **Forced structured output** (line 115): `tool_choice={"type": "tool",
  ...
  `_TOOL` schema (line 18) rather than free text — you get back
...
- `extract_amount_line(summary)` (line 50) — finds the dedicated
  ...
  or the Note line) rather than a whole-text regex scan — a dollar figure
  mentioned inside "What it says" (e.g. a CPF account balance) shouldn't
  be mistaken for the actual action amount.
